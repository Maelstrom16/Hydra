...
use crate::{
    common::errors::HydraIOError, deserialize, emulator::gameboy::{
        GbModel, apu::{Apu, channel::{Noise, Pulse, PulseType, Wave}, state::ApuState}, interrupt::{InterruptEnable, InterruptFlags}, joypad::Joypad, memory::{hdma::HdmAccessor, oam::Oam, rom::{Rom, RomHeader}, vram::Vram, wram::Wram}, ppu::{PpuMode, colormap::{self, ColorMap}, state::PpuState}, serial::SerialConnection, timer::MasterTimer
    }, graphics::Graphics, input::{ControllerState, SdlContainer}, serialize, window::UserEvent
};
...
impl<M: GbModel> MemoryMap<M> {
    pub fn new(model: &M, cgb_mode: bool, controllers: Arc<RwLock<ControllerState>>, graphics: Arc<RwLock<Graphics>>, proxy: EventLoopProxy<UserEvent>) -> Result<Self, HydraIOError> {
        let interrupt_flags = InterruptFlags::new();
        let interrupt_enable = InterruptEnable::new();
        let joypad = Joypad::new::<M>(controllers);
        let serial = SerialConnection::new(cgb_mode);
        let vram = Vram::new::<M>();
        let wram = Wram::new::<M>();
    ...
    fn is_cart_accessible(&self) -> bool {
        ...
        matches!(self.dma_cycle, None) || (M::is_color() && !(0x0000..=0x7FFF).contains(&((self.dma_source as u16) << 8)))
    }
    ...
    fn is_wram_accessible(&self) -> bool {
        ...
        matches!(self.dma_cycle, None) || (M::is_color() && !(0xC000..=0xDFFF).contains(&((self.dma_source as u16) << 8)))
    }
    ...
    pub fn read_u8(&self, address: u16, is_dma: bool) -> u8 {
        let read_result = match address {
            ...
            0x8000..=0x9FFF if is_dma || self.is_vram_accessible() => self.vram.read_u8(address, self.is_cgb_mode()),
            ...
            0xC000..=0xDFFF if is_dma || self.is_wram_accessible() => Ok(self.wram.read_u8(address, self.is_cgb_mode())),
            0xE000..=0xFFFF if is_dma => Ok(self.wram.read_u8(address - 0x2000, self.is_cgb_mode())), // OAM DMA echo RAM mirrors the entirety of WRAM
            0xE000..=0xFDFF if self.is_wram_accessible() => Ok(self.wram.read_u8(address - 0x2000, self.is_cgb_mode())), // Treat exactly like WRAM
            0xFE00..=0xFEFF if self.is_oam_accessible() => self.oam.read(address),
            0xFF00 => Ok(self.joypad.read_joyp()),
            0xFF01..=0xFF02 => self.serial.read(address, self.is_cgb_mode()),
            0xFF04..=0xFF07 | 0xFF4D => self.timer.read(address, self.is_cgb_mode()),
            ...
            0xFF72..=0xFF73 if M::is_color() => Ok(self.cgb_misc[address as usize - 0xFF72]),
            0xFF74 if self.is_cgb_mode() => Ok(self.cgb_misc[2]),
            0xFF75 if M::is_color() => Ok(self.cgb_misc[3] | 0b10001111),
    ...
    pub fn write_u8(&mut self, val: u8, address: u16) -> () {
        let write_result = match address {
            ...
            0x8000..=0x9FFF if self.is_vram_accessible() => self.vram.write_u8(val, address, self.is_cgb_mode()),
            ...
            0xC000..=0xDFFF if self.is_wram_accessible() => Ok(self.wram.write_u8(val, address, self.is_cgb_mode())),
            0xE000..=0xFDFF if self.is_wram_accessible() => Ok(self.wram.write_u8(val, address - 0x2000, self.is_cgb_mode())), // Treat exactly like WRAM
            ...
            0xFF01..=0xFF02 => self.serial.write(val, address, self.is_cgb_mode()),
            0xFF04..=0xFF07 | 0xFF4D => self.timer.write(val, address, self.is_cgb_mode(), &mut self.apu_state),
            ...
            0xFF72..=0xFF73 if M::is_color() => Ok(self.cgb_misc[address as usize - 0xFF72] = val),
            0xFF74 if self.is_cgb_mode() => Ok(self.cgb_misc[2] = val),
            0xFF75 if M::is_color() => Ok(self.cgb_misc[3] = val | 0b10001111),
